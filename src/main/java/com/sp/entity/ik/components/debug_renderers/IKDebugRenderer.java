...
public interface IKDebugRenderer<E extends IKAnimatable<E>, C extends IKModelComponent<E>> {
    ...
    static void drawBox(PoseStack matrices, MultiBufferSource vertexConsumers, Vec3 targetPos, Entity entity, int red, int green, int blue, int alpha) {
        Vec3 offsetEntityPos = entity.position().add(0.5, 0.5, 0.5);
        DebugRenderer.renderFilledBox(matrices, vertexConsumers, AABB.unitCubeFromLowerCorner(targetPos).deflate(0.65, 0.65, 0.65).move(-offsetEntityPos.x, -offsetEntityPos.y, -offsetEntityPos.z), (float) red / 255, (float) green / 255, (float) blue / 255, (float) alpha / 255);
    }
